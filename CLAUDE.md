...
*Regra de Falha Silenciosa:* "Falha silenciosa para o cliente final" significa não quebrar a criação/fluxo do booking B2C e não exibir erros técnicos ao visitante. NÃO significa esconder do owner, omitir Sentry Issue, omitir Sentry Log, omitir auditoria em `disparos_whatsapp` ou deixar de alertar o profissional.

**Observabilidade da mensageria já está instrumentada — use o que existe, não reimplemente** (baseline: quick task `260724-observabilidade-mensageria`, PR #11 / `a9d071a`). Quatro pilares, cada um respondendo a uma pergunta diferente: Sentry Issue (falha acionável), Sentry Log estruturado via `logOperacional` (`src/lib/observabilidade/log.ts`, allowlist fechada de atributos), PostHog (taxa agregada) e `disparos_whatsapp` (auditoria append-only por tenant). Regras que valem para código novo: em Server Action, webhook ou route handler que pode encerrar logo depois, use as variantes **aguardadas** `reportarExcecaoAguardando`/`reportarFalhaSilenciosaAguardando` (`Sentry.flush`) — a versão fire-and-forget perde o evento quando o processo congela; a mensagem da Issue é **sintética e estática** (`whatsapp:evolution_http_error`, `qstash:publish_http_error`, …) para não estilhaçar o agrupamento; identificador de tenant/agendamento só entra pseudonimizado (`tenantHash`/`agendamentoHash`, `src/lib/observabilidade/hash.ts`). Mapa completo de estados em `docs/06-MENSAGERIA_E_WHATSAPP.md` e no `INCIDENT.md` da quick task.
...
Tabelas: `perfis_empresas` (identidade + slug público + configs de agendamento: `antecedencia_minima_minutos`, `horizonte_maximo_dias`, `timezone`; + personalização **exclusiva do Pro**: `cor_marca`/`logo_url`/`capa_url`, e infos livres `instagram`/`endereco`), `servicos`, `horarios_funcionamento` (dia_semana 0–6, **N janelas por dia**; semana salva de uma vez pela RPC atômica `substituir_horarios_funcionamento`, sobreposição validada na action via `src/lib/horarios.ts`), `excecoes_agenda` (feriados/bloqueios), `whatsapp_configs` (instância Evolution + templates), `clientes`, `agendamentos` (status: pendente/confirmado/concluido/cancelado), `assinaturas` (planos plus/pro; gratuito = sem linha vigente; **Plus caminha para descontinuação** — não ganha recursos novos), `disparos_whatsapp` (log append-only de auditoria de mensageria — sem conteúdo nem telefone).

Storage: bucket público `imagens-perfis` (logo/capa dos tenants, paths `<org_id>/…`). **Sem RLS em `storage.objects`** (a role postgres não é owner neste projeto — não consegue criar políticas lá): a API do Storage é default-deny e toda escrita passa pelas actions de `src/app/actions/imagens-perfil.ts` (auth + gating Pro + validação + `createAdminClient()`). Migrations de Storage são manuais (ver `docs/03` e `docs/SUPABASE_DECLARATIVE-DATABASE-SCHEMA.md`). A página pública consome a personalização **sanitizada pelo plano vigente** via `obterDadosBookingPublico` (chave `personalizacao`) — nunca ler `cor_marca`/`logo_url`/`capa_url` cruas em UI pública.
