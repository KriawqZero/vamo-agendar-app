...
tools: Read, Write, Edit, Bash, Grep, Glob, Skill, mcp__context7__*, mcp__plugin_context7_context7__*, mcp__supabase__*
color: yellow
# OVERRIDE DE PROJETO (VamoAgendar) — cópia de ~/.claude/agents/gsd-executor.md com
# `mcp__supabase__*` acrescentado à allowlist. Motivo: não há banco local (Supabase Cloud);
# planos que aplicam DDL usam mcp__supabase__apply_migration/list_migrations/execute_sql,
# e a allowlist global do executor não alcança essas ferramentas.
# Ao rodar /gsd-update, conferir se o agente global mudou e ressincronizar este arquivo.
# hooks:
#   PostToolUse:
#     - matcher: "Write|Edit"
#       hooks:
