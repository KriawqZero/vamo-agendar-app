...
export default function BookingWizard({ perfil, servicos }: BookingWizardProps) {
    ...
    const [agendamentoCriado, setAgendamentoCriado] = useState<{ data_hora: string; id: string } | null>(null)
    ...
    const handleConfirmarAgendamento = async (e: React.FormEvent) => {
        ...
        if (!telLimpo && !email.trim()) {
            setErroSubmit('Forneça pelo menos um meio de contato (WhatsApp ou E-mail) para receber confirmações.')
            return
        }
    ...
    return (
        <div className="max-w-xl mx-auto bg-white dark:bg-zinc-900 border border-zinc-200 dark:border-zinc-800 rounded-3xl overflow-hidden shadow-xl">
            ...
            <div className="bg-gradient-to-br from-zinc-900 to-zinc-800 dark:from-zinc-950 dark:to-zinc-900 p-6 text-white text-center border-b border-zinc-800">
                <span className="text-xs font-bold uppercase tracking-widest text-zinc-400">Agendamento Online</span>
            ...
            {etapa !== 'sucesso' && (
                <div className="flex border-b border-zinc-100 dark:border-zinc-800/80 bg-zinc-50 dark:bg-zinc-900/50">
                    ...
                    <button
                        onClick={() => servicoSelecionado && horarioSelecionado && setEtapa('contato')}
            ...
            {etapa === 'servico' && (
                <div className="p-6 space-y-4 animate-in fade-in duration-200">
                    <div className="space-y-1">
                        <h2 className="text-base font-bold text-zinc-900 dark:text-zinc-50">Escolha o Serviço</h2>
                        <p className="text-xs text-zinc-500">Selecione uma opção para ver os horários disponíveis.</p>
                    </div>
                        ...
                        {servicos.map((servico) => (
                            ...
                            >
                                <div className="space-y-1">
                                    ...
                                    {servico.descricao && (
                                        <p className="text-xs text-zinc-500 line-clamp-1">{servico.descricao}</p>
                                    )}
            ...
            {etapa === 'data_hora' && (
                <div className="p-6 space-y-5 animate-in fade-in duration-200">
                    <div className="flex items-center gap-2">
                        ...
                        <div>
                            <h2 className="text-base font-bold text-zinc-900 dark:text-zinc-50">Quando deseja agendar?</h2>
                            <p className="text-xs text-zinc-500">
                                Serviço: <span className="font-bold">{servicoSelecionado?.nome}</span>
                            </p>
                        </div>
                    </div>
                    ...
                    <div className="flex gap-2 overflow-x-auto pb-2 scrollbar-none snap-x">
                        {datasDisponiveis.map((dt) => (
                            ...
                            >
                                ...
                                <span className="text-sm font-extrabold mt-0.5">{dt.label.split('/')[0]}</span>
                            </button>
                        ))}
                    </div>
                    ...
                    {/* Grid de Horários */}
                    <div className="space-y-3">
                        <span className="text-xs font-bold uppercase text-zinc-400 block">Horários Disponíveis</span>
                        ...
                        {carregandoSlots ? (
                            ...
                            <div className="text-center py-6 text-zinc-400 dark:text-zinc-500 bg-zinc-50 dark:bg-zinc-900/50 rounded-2xl text-xs font-medium border border-dashed border-zinc-200 dark:border-zinc-800">
                                Sem horários livres para este dia. Tente selecionar outra data acima.
                            </div>
                        ) : (
                            <div className="grid grid-cols-3 gap-2">
                                {slots.map((slot) => (
            ...
            {etapa === 'contato' && (
                <div className="p-6 space-y-5 animate-in fade-in duration-200">
                    <div className="flex items-center gap-2">
                        ...
                        <div>
                            <h2 className="text-base font-bold text-zinc-900 dark:text-zinc-50">Finalizar Agendamento</h2>
                            <p className="text-xs text-zinc-500">
                    ...
                    <div className="bg-zinc-50 dark:bg-zinc-900/50 border border-zinc-200/50 dark:border-zinc-800/50 p-4 rounded-2xl text-xs space-y-2">
                        ...
                        <div className="flex justify-between">
                            ...
                            <span className="font-bold text-zinc-950 dark:text-zinc-100">{servicoSelecionado?.duracao_minutos} min</span>
                        </div>
                        <div className="flex justify-between">
                            <span className="text-zinc-400">Preço:</span>
                            <span className="font-bold font-mono text-zinc-900 dark:text-zinc-50">
                                {Number(servicoSelecionado?.preco).toLocaleString('pt-BR', { style: 'currency', currency: 'BRL' })}
                            </span>
                        </div>
                        <div className="flex justify-between border-t border-zinc-100 dark:border-zinc-800/80 pt-2">
                            ...
                            <span className="font-bold text-zinc-900 dark:text-zinc-50 text-right">
                                {horarioSelecionado ? formatarDataHoraFinal(horarioSelecionado) : ''}
                            </span>
                        </div>
                    </div>
                    ...
                    <form onSubmit={handleConfirmarAgendamento} className="space-y-4">
                        ...
                        <div className="space-y-1">
                            <label className="text-xs font-bold uppercase text-zinc-400 block">Seu Nome Completo</label>
                            <input
                                type="text"
                                value={nome}
                        ...
                        <div className="space-y-1">
                            <label className="text-xs font-bold uppercase text-zinc-400 block">WhatsApp / Telefone</label>
                            <input
                                type="tel"
                                value={telefone}
                                ...
                                className="w-full px-3.5 py-2 border border-zinc-200 dark:border-zinc-800 rounded-lg text-sm bg-zinc-50 dark:bg-zinc-900 outline-hidden text-zinc-900 dark:text-zinc-50 font-medium"
                            />
                            <p className="text-[10px] text-zinc-400">Para receber lembretes e avisos de confirmação.</p>
                        </div>

                        <div className="space-y-1">
                            <label className="text-xs font-bold uppercase text-zinc-400 block">E-mail (Opcional)</label>
                            <input
                                type="email"
                                value={email}
            ...
            {etapa === 'sucesso' && (
                <div className="p-8 text-center space-y-6 animate-in zoom-in-95 duration-300">
                    ...
                    <div className="space-y-2">
                        <h2 className="text-lg font-extrabold text-zinc-900 dark:text-zinc-50">Agendamento Realizado!</h2>
                    ...
                    <div className="bg-zinc-50 dark:bg-zinc-900/50 border border-zinc-200/60 dark:border-zinc-800 p-5 rounded-2xl text-sm space-y-3 max-w-sm mx-auto text-left">
                        <div>
                            <span className="text-[10px] font-bold uppercase tracking-wider text-zinc-400 block">Estabelecimento</span>
                            <span className="font-bold text-zinc-900 dark:text-zinc-50">{perfil.nome_estabelecimento}</span>
                        </div>
                        <div>
                            <span className="text-[10px] font-bold uppercase tracking-wider text-zinc-400 block">Serviço</span>
                            <span className="font-bold text-zinc-900 dark:text-zinc-50">{servicoSelecionado?.nome}</span>
                        </div>
                        <div>
                            <span className="text-[10px] font-bold uppercase tracking-wider text-zinc-400 block">Data e Horário</span>
                            <span className="font-bold text-zinc-900 dark:text-zinc-50">
                                {agendamentoCriado ? formatarDataHoraFinal(agendamentoCriado.data_hora) : ''}
                            </span>
                        </div>
                    </div>
